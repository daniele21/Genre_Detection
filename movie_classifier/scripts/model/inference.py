...
def genre_prediction(model,
                     dataset,
                     tokenizer: MyTokenizer):

    y_pred, y_bin_pred = model_inference(model, dataset)

    genres = extract_five_movies(y_pred, tokenizer)

    most_prob_genre = [x.split()[0] for x in genres]

    return most_prob_genre[0]
